...
io.on("connection", (socket) => {
  ...
  socket.on("requestLiveTranscription", () => {
    liveCheckInterval = setInterval(async () => {
      // Check for new transcriptions since last update
  ...
  socket.on("disconnect", () => {
    ...
    // Remove the client's length checking for the sessionId
    const index = checkLengthsArray.findIndex((c) => c.sessionId === sessionId);
    if (index !== -1) {
      checkLengthsArray.splice(index, 1);
    }
    // if all clients with the same sessionId are disconnected, clear the interval
    const sessionIds = checkLengthsArray.map((c) => c.sessionId);
